...
def download_H8data(date):
    ...
    try :
        ...
        scp.get(file_path(1, date), folder_original+'/')
        os.system('lbzip2 -d {}{}'.format(folder_original+'/',file_path(1, date)[-33:]))
...
lon_y=np.linspace(120,149.99,3000)
    ...
    for m in range(len(DD)):
        for i in range(len(HH)):
            for j in range(len(MIN)):
                ...
                if  os.path.exists(folder_original+'/'+date+'.vis.01.fld.geoss'):
                    
                    # read himawari8 file
                    with open(folder_original+'/'+date+'.vis.01.fld.geoss','rb') as fp:
                    ...
                    # Using LUT
                    SR = Parallel(n_jobs=-1)(delayed(calculate_6s_band1)(i) for i in range(3000))
