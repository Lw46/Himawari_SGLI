...
VZA_path = 'view_zM_JAPAN_10.dat'
VAZ_path = 'view_aM_JAPAN_10.dat'
LUT_path = '/media/liwei/Data/LUT_v01/'
...
def download_H8data(date):
    ...
    try :
        ...
        scp.get(file_path(2, date), folder_original+'/')
        os.system('lbzip2 -d {}{}'.format(folder_original+'/',file_path(2, date)[-33:]))
...
def griddata_inter(X1,X2,X3,b,c,d):
    X1_new_inter=[]
    X2_new_inter=[]
    X3_new_inter=[]
    
    X1_inter=X1[b,c,:,:,d].reshape(17*17,1)
    X2_inter=X2[b,c,:,:,d].reshape(17*17,1)
    X3_inter=X3[b,c,:,:,d].reshape(17*17,1)
...
def calculate_6s_band2(i):
    ...
    xi=np.array([OZ_input,AOT550_input,RAA_input,SZA_input,view_zM_input])
    xi=xi.T
    xa=fn1(xi)
    xb=fn2(xi)
    xc=fn3(xi)
...
with open(VZA_path,'rb') as fp:
    view_zM = np.fromstring(fp.read()).reshape(3000,3000) 
with open(VAZ_path,'rb') as fp:
    view_aM = np.fromstring(fp.read()).reshape(3000,3000)     

# read LUT
outfile1 = LUT_path + '01_band1.csv'
outfile2 = LUT_path + '02_band1.csv'
outfile3 = LUT_path + '03_band1.csv'
...
# reshape LUT
X1_reshape=X1.reshape(5,12,17,17,19)
X2_reshape=X2.reshape(5,12,17,17,19)
X3_reshape=X3.reshape(5,12,17,17,19)
del X1,X2,X3

# SZA,SAZ,VZA interpolation
point=get_point()
xi,yi=np.ogrid[0:80:161j, 0:80:161j]
output = Parallel(n_jobs=-1)(delayed(griddata_inter)(X1_reshape,X2_reshape,X3_reshape,b,c,d)                             for b in range(len(ozone))                              for c in range(len(AOT))                              for d in range(len(raa)))

X1_new_inter_reshape=np.array(output)[:,0].reshape(5,12,19,161,161)
X2_new_inter_reshape=np.array(output)[:,1].reshape(5,12,19,161,161)
X3_new_inter_reshape=np.array(output)[:,2].reshape(5,12,19,161,161)
...
fn1 = RegularGridInterpolator((ozone,AOT,raa,sza_new,vza_new),X1_new_inter_reshape,bounds_error=False,fill_value=np.nan)  
fn2 = RegularGridInterpolator((ozone,AOT,raa,sza_new,vza_new),X2_new_inter_reshape,bounds_error=False,fill_value=np.nan)  
fn3 = RegularGridInterpolator((ozone,AOT,raa,sza_new,vza_new),X3_new_inter_reshape,bounds_error=False,fill_value=np.nan)

lat_x=np.linspace(50,20.01,3000)
lon_y=np.linspace(120,149.99,3001)
    ...
    for m in range(len(DD)):
        for i in range(len(HH)):
            for j in range(len(MIN)):
                ...
                if  os.path.exists(folder_original+'/'+date+'.vis.02.fld.geoss'):
                    
                    # read himawari8 file
                    with open(folder_original+'/'+date+'.vis.02.fld.geoss','rb') as fp:
                        data = np.fromstring(fp.read(),dtype='>u2').reshape(12000,12000)
                        data=data[1000:4000,3500:6500]
                        data=DN2tbb(data)
                        data=data/100
                    ...
                    # Solar angle                    
                    with open(SZA_path + 'solar_zM_' + date + '.dat','rb') as fp:
                        Solar_zM = np.fromstring(fp.read()).reshape(3000,3000) 
                    with open(SAZ_path + 'solar_aM_' + date + '.dat','rb') as fp:
                    ...
                    # Using LUT
                    SR = Parallel(n_jobs=-1)(delayed(calculate_6s_band2)(i) for i in range(3000))

                
                    # Save file and remove download input data
                    SR=np.array(SR).reshape(3000,3000)
                    SR_file=open(folder_AC+'/'+date+'_b02.dat','wb')
