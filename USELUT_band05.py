...
VZA_path = 'view_zM_JAPAN_20.dat'
VAZ_path = 'view_aM_JAPAN_20.dat'
LUT_path = ''
...
def download_H8data(date):
    ...
    try :
        ...
        scp.get(file_path(5, date), folder_original+'/')
        os.system('lbzip2 -d {}{}'.format(folder_original+'/',file_path(5, date)[-33:]))
...
def griddata_inter(X1,X2,X3,a,c,d):
    X1_new_inter=[]
    X2_new_inter=[]
    X3_new_inter=[]
    
    X1_inter=X1[a,c,:,:,d].reshape(17*17,1)
    X2_inter=X2[a,c,:,:,d].reshape(17*17,1)
    X3_inter=X3[a,c,:,:,d].reshape(17*17,1)
...
def calculate_6s_band5(i):
    ...
    xi=np.array([WV_input,AOT550_input,RAA_input,SZA_input,view_zM_input])
    xi=xi.T
    xa=fn1(xi)
    xb=fn2(xi)
    xc=fn3(xi)
...
# read LUT
outfile1 = LUT_path + '01_band5.csv'
outfile2 = LUT_path + '02_band5.csv'
outfile3 = LUT_path + '03_band5.csv'
...
# reshape LUT
X1_reshape=X1.reshape(8,12,17,17,19)
X2_reshape=X2.reshape(8,12,17,17,19)
X3_reshape=X3.reshape(8,12,17,17,19)
del X1,X2,X3

# SZA,SAZ,VZA interpolation
point=get_point()
xi,yi=np.ogrid[0:80:161j, 0:80:161j]
output = Parallel(n_jobs=-1)(delayed(griddata_inter)(X1_reshape,X2_reshape,X3_reshape,a,c,d)                             for a in range(len(water))                              for c in range(len(AOT))                              for d in range(len(raa)))
...
lat_x=np.linspace(50,20,1500)
lon_y=np.linspace(120,150,1500)
    ...
    for m in range(len(DD)):
        for i in range(len(HH)):
            for j in range(len(MIN)):
                ...
                if  os.path.exists(folder_original+'/'+date+'.sir.01.fld.geoss'):
                    
                    # read himawari8 file
                    with open(folder_original+'/'+date+'.sir.01.fld.geoss','rb') as fp:
                        data = np.fromstring(fp.read(),dtype='>u2').reshape(6000,6000)
                        data=data[500:2000,1750:3250]
                        data=DN2tbb(data)
                        data=data/100
                    ...
                    Solar_aM=cv2.resize(np.array(Solar_aM),(1500,1500),interpolation=cv2.INTER_NEAREST)
                    Solar_zM=cv2.resize(np.array(Solar_zM),(1500,1500),interpolation=cv2.INTER_NEAREST)
                    ...
                    # Using LUT
                    SR = Parallel(n_jobs=-1)(delayed(calculate_6s_band5)(i) for i in range(1500))
