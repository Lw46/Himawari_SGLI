...
def download_H8data(date):
    ...
    try :
        ...
        scp.get(file_path(6, date), folder_original+'/')
        os.system('lbzip2 -d {}{}'.format(folder_original+'/',file_path(6, date)[-33:]))
...
outfile3 = LUT_path + '03_band6.csv'
    ...
    for m in range(len(DD)):
        for i in range(len(HH)):
            for j in range(len(MIN)):
                ...
                if  os.path.exists(folder_original+'/'+date+'.sir.02.fld.geoss'):
                    
                    # read himawari8 file
                    with open(folder_original+'/'+date+'.sir.02.fld.geoss','rb') as fp:
                    ...
                    # Using LUT
                    SR = Parallel(n_jobs=-1)(delayed(calculate_6s_band6)(i) for i in range(1500))
