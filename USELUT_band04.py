...
def download_H8data(date):
    ...
    try :
        ...
        scp.get(file_path(4, date), folder_original+'/')
        os.system('lbzip2 -d {}{}'.format(folder_original+'/',file_path(4, date)[-33:]))
...
# read LUT
outfile1 = LUT_path + '01_band4.csv'
outfile2 = LUT_path + '02_band4.csv'
outfile3 = LUT_path + '03_band4.csv'
    ...
    for m in range(len(DD)):
        for i in range(len(HH)):
            for j in range(len(MIN)):
                ...
                if  os.path.exists(folder_original+'/'+date+'.vis.03.fld.geoss'):
                    
                    # read himawari8 file
                    with open(folder_original+'/'+date+'.vis.03.fld.geoss','rb') as fp:
                    ...
                    # Using LUT
                    SR = Parallel(n_jobs=-1)(delayed(calculate_6s_band4)(i) for i in range(3000))
